...
import type { Config, TimeRange, ChartComment, ExportFormat } from "@/lib/types"
...
export function AdvancedChartView({
  ...
  const createYAxisConfig = () => {
    ...
    if (mode === "split") {
      ...
      return createAxisConfiguration(configs)
    }
  }
  ...
  const handleExportChart = (format: ExportFormat) => {
    ...
    try {
      if (format === "pdf") {
        // Create a new window with only the chart for PDF export
      ...
      // For SVG, PNG, JPG - use canvas-based approach
  ...
  const handleExportIndividualChart = (format: ExportFormat, configId: string) => {
    ...
    try {
      if (format === "pdf") {
        // Create a new window with only the specific chart for PDF export
        const chartContainer = chart.container
        const chartHTML = chartContainer.outerHTML
